...
_ft_list_sort:
	test rdi, rdi
	jz _return_nothing_list_merge ; returns ; if t_list ** is NULL
	test rsi, rsi
	jz _return_nothing_list_merge ; returns ; if the cmp function pointer is NULL
	mov rax, QWORD[rdi]
	test rax, rax
	jz _return_nothing_list_merge ; returns ; if the list size is 0
	cmp QWORD[rax + 8], 0
	je _return_nothing_list_merge ; returns ; if the list size is 1
	push rbp
	mov rbp, rsp
	sub rsp, 24
...
_split_list_sort:
	...
	_loop_split_list_sort:
		mov rax, QWORD[rsp + 8]
		test rax, rax
		jz _assign_split_list_sort
		push rdi
		mov rdi, QWORD[rax + 8]
		...
		pop rdi
		jmp _loop_split_list_sort
	_assign_split_list_sort:
		mov QWORD[rsi], rdi
		mov rax, QWORD[rsp]
		push rdi
		mov rdi, QWORD[rax + 8]
